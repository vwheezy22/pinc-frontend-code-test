...
var name=state.file.opts.filename;var files=[];var dir=_path3.default.join(_path3.default.dirname(name),src);// path of the target dir.
for(var i=node.specifiers.length-1;i>=0;i--){dec=node.specifiers[i];if(t.isImportNamespaceSpecifier(dec)&&_fs3.default.existsSync(dir)&&!_fs3.default.statSync(dir).isFile()){addWildcard=true;wildcardName=node.specifiers[i].local.name;node.specifiers.splice(i,1)}// This handles { A, B, C } from 'C/*'
...
node.specifiers.splice(i,1)}}// If they are no specifies but it is explicit
if(isExplicitWildcard){addWildcard=true;wildcardName=null}// All the extensions that we should look at
...
if(filterNames.length===0&&wildcardName!==null){var obj=t.variableDeclaration('const',[t.variableDeclarator(t.identifier(wildcardName),t.objectExpression([]))]);path.insertBefore(obj)}// Will throw if the path does not point to a dir
try{var r=_fs3.default.readdirSync(dir);for(var i=0;i<r.length;i++){// Check extension is of one of the aboves
var _path$parse=_path3.default.parse(r[i]),_name=_path$parse.name,ext=_path$parse.ext;if(exts.indexOf(ext.substring(1))>-1&&filenameRegex.test(_name)){files.push(r[i])}}}catch(e){console.warn('Wildcard for '+name+' points at '+src+' which is not a directory.');return}// This is quite a mess but it essentially formats the file
...
if(state.opts.noModifyCase!==true){var parts=fancyName.match(/[A-Z][a-z]+(?![a-z])|[A-Z]+(?![a-z])|([a-zA-Z\d]+(?=-))|[a-zA-Z\d]+(?=_)|[a-z]+(?=[A-Z])|[A-Za-z0-9]+/g);if(state.opts.useCamelCase){fancyName=parts[0].toLowerCase()+parts.slice(1).map(function(s){return s[0].toUpperCase()+s.substring(1)}).join('')}else{fancyName=parts.map(function(s){return s[0].toUpperCase()+s.substring(1)}).join('')}}// Now we're 100% settled on the fancyName, if the user
